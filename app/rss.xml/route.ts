...
export async function GET() {
  ...
  const items = (posts ?? []).map((post) => `<item><title>${escapeXml(post.title ?? "未命名文章")}</title><link>${baseUrl}/posts/${post.slug}</link><guid>${baseUrl}/posts/${post.slug}</guid><description>${escapeXml(post.description ?? "")}</description><pubDate>${new Date(post.updated_at ?? post.created_at ?? Date.now()).toUTCString()}</pubDate></item>`).join("");
